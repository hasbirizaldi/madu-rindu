...
const App = () => {
  ...
  return (
    <div className="bg-[var(--color-honey-light)] text-gray-800 font-[Poppins]">
      {/* 🟤 Navbar */}
      <nav className="bg-honey-deep flex justify-between items-center px-3 md:px-10 py-1 backdrop-blur-md shadow-md fixed w-full top-0 z-50">
        {/* Logo */}
        <a href="#hero" className="flex items-center gap-3">
          <img src={images.logo} alt="Madu Rindu" className="w-16 md:w-24" />
        ...
        <div className="hidden md:block">
          <button className="bg-white flex items-center justify-center gap-2 cursor-pointer font-poppins font-semibold px-6 py-2 rounded-full shadow-md hover:bg-amber-100 transition-transform hover:scale-[1.01] text-honey-deep text-sm">
            <FaCartShopping />
            Pesan Sekarang
          </button>
        </div>
      ...
      {/* 🍯 Hero Section */}
      <section id="hero" className="text-center flex flex-col items-center justify-center bg-hero bg-cover bg-center h-[100vh]  px-4 md:px-0">
        <div className="bg-honey-deep/90 p-2 py-5 md:p-6 rounded-3xl shadow-lg backdrop-blur-xs max-w-2xl">
          ...
          <button className="mt-8 md:mt-10 bg-amber-50 text-honey-deep font-poppins font-semibold px-8 md:px-10 py-3 rounded-full shadow-lg hover:bg-amber-100 transition-transform hover:scale-[1.01] cursor-pointer">
            Dapatkan Sekarang
          </button>
        </div>
      </section>
      ...
      {/* 🌻 Keunggulan */}
      <section id="keunggulan" className="py-20 md:py-24 bg-honey text-center px-6">
        <h2 className="text-3xl font-bold text-honey-deep mb-10 md:mb-12 font-poppins">Keunggulan Madu Rindu</h2>
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {testimonies.map((item, i) => (
            <div key={i} className=" bg-amber-100 border-2 border-amber-200 rounded-2xl shadow-md hover:scale-[1.01] cursor-pointer hover:shadow-gray-900/30  transition-transform overflow-hidden ">
              <div className=" bg-amber-200">
                <img src={item.image} alt="testimony" className="w-full h-70 mx-auto object-cover rounded shadow-lg" />
              </div>
              <div className="bg-honey-deep py-4 px-2 h-35 border-t-2 border-amber-50">
      ...
      <section id="produk" className="py-20 md:py-24 bg-[var(--color-honey-light)] px-6 text-center">
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {products.map((item, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-md overflow-hidden hover:scale-[1.01] hover:shadow-gray-900/30 transition-transform">
      ...
      {/* 🧡 Testimoni */}
      <section id="testimoni" className="py-20 md:py-24 bg-honey text-center px-6">
      ...
      {/* 💛 CTA */}
      <section id="pesan" className="bg-honey text-honey-deep text-center py-20 px-6">
        ...
        <button className="bg-honey-deep text-white cursor-pointer font-semibold px-8 md:px-10 py-3 rounded-full shadow-md hover:bg-amber-100 transition-transform hover:scale-[1.01]">Pesan Sekarang</button>
      </section>

      {/* 🟤 Footer */}
      <footer className="bg-honey-deep text-amber-100 text-center py-3 text-xs md:text-base">
        <p>
          &copy; 2025 <span className="font-semibold  text-white">Madu Rindu</span> | Dibuat dengan ❤
          <a href="https://brewokode.com/" className="underline hover:text-amber-300 ml-2">
            Brewokode.com
          </a>
        </p>
      </footer>
